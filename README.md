...
- **Code Generation** - Generate boilerplate code, migrations, APIs using Handlebars templates
...
| **@xomda/template**          | Template Engine & Plugin System      | Handlebars engine, template packages, external plugins |  Complete        |
...
- **Advanced Template Engine**: Handlebars with custom helpers, multi-scope generation (Entity/Enum/Package/Model)
...
│   │   ├── src/engine.ts        # Handlebars compilation
│   │   ├── src/helpers.ts       # Custom helpers (camelCase, etc.)
│   │   └── src/storage.ts       # File I/O for templates
...
Templates can now generate code at different levels of the model hierarchy:

- **`Entity` scope**: One file per entity (classes, models, schemas)
- **`Enum` scope**: One file per enum (type definitions, constants)
- **`Package` scope**: One file per package (module configuration, barrel exports, package metadata)
- **`Model` scope** (default): Single file for entire model (database schema, config, migration script)

### Handlebars Templates

Write templates in the UI (TemplatesView) to generate code. Include YAML frontmatter to specify scope:
...
package {{model.name | lowerCase}}.entities;

public class {{name | pascalCase}} {
{{#each attributes}}
  private {{this.type}} {{this.name}};  // {{#if this.required}}required{{/if}}
{{/each}}
}
```
...
- `outputPath`: Handlebars expression for output file path
- `language`: Syntax hint for editor ("java", "typescript", "sql", etc.)
- `scope`: Render scope - `'Entity'`, `'Enum'`, `'Package'`, or omit for `'Model'`
...
- `javaType`: Convert type names to Java types
- `isPrimitive`: Check if type is primitive

### Examples

**Entity Scope - Java Classes**:
...
**Enum Scope - TypeScript Enums**:

```handlebars
---
name: 'TypeScript Enum'
scope: 'Enum'
outputPath: 'types/{{pascalCase name}}.ts'
language: 'typescript'
---
...
**Package Scope - Module Exports**:

```handlebars
---
name: 'Barrel Export'
scope: 'Package'
outputPath: '{{name}}/index.ts'
language: 'typescript'
---
...
| **Template Engine**   | Handlebars           | 4.7.9   |
