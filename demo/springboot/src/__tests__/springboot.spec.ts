...
describe('springboot demo — JPA entities', () => {
  ...
  it('Author entity has unique email column', async () => {
    ...
    expect(src).toMatch(/@Column\(name = "email", nullable = false, unique = true\)\s*\n\s*private String email;/)
  })
...
describe('springboot demo — repositories & services', () => {
  it.each(['Author', 'Post', 'Comment'])('%sRepository extends JpaRepository', async (entity) => {
    ...
    expect(src).toMatch(new RegExp(`@Repository\\s+public interface ${entity}Repository extends JpaRepository<${entity}, UUID>`))
  })

  it.each(['Author', 'Post', 'Comment'])('%sService delegates CRUD to repository', async (entity) => {
...
describe('springboot demo — infrastructure files', () => {
  ...
  it('generates a pom.xml targeting Spring Boot 4', async () => {
    ...
    expect(src).toMatch(/<artifactId>spring-boot-starter-parent<\/artifactId>\s*<version>4\.0\.0<\/version>/)
