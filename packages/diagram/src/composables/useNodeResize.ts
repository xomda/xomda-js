...
export interface UseNodeResizeOptions {
  ...
  zoom: () => number
  /**
   * Minimum width and height in CSS pixels (pre-zoom), evaluated on every
   * pointer-move so it can track reactive content (e.g. a package's
   * smallest enclosing rectangle). Default 96 on both axes.
   */
 ...
 */
const DEFAULT_MIN_SIZE = { width: 96, height: 96 }

export function useNodeResize(options: UseNodeResizeOptions): UseNodeResizeReturn {
  const getMinSize = () => options.minSize?.() ?? DEFAULT_MIN_SIZE
  ...
  function onPointerDown(e: PointerEvent): void {
    ...
    startSize.width = layout.width ?? options.el.value?.offsetWidth ?? min.width
    startSize.height = layout.height ?? options.el.value?.offsetHeight ?? min.height
  ...
  function onPointerMove(e: PointerEvent): void {
    ...
    const min = getMinSize()
    const newW = snap(Math.max(min.width, startSize.width + dx))
    const newH = snap(Math.max(min.height, startSize.height + dy))
