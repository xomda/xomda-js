import { sortAttributesForDisplay } from '@xomda/core'
import { EntityIcon } from '@xomda/icons'
import { computed, defineComponent, type PropType, ref, type SlotsType, type VNode } from 'vue'
...
export const Entity = defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    // Display order pins `description` to the end (convention).
    const displayAttributes = computed(() => sortAttributesForDisplay(props.entity.attributes))
    const inheritedDisplayAttributes = computed(() =>
      sortAttributesForDisplay(props.inheritedAttributes)
    )
      ...
      return (
        ...
        >
          ...
          <div class={styles.attributes}>
            {slots['attrs-before']?.()}
            {inheritedDisplayAttributes.value.map((attribute) => (
            ...
            {displayAttributes.value.map((attribute, index) => {
              // `description` is pinned last by convention — not draggable and
              // not a drop target. Everything else reorders normally.
              const isDescription = attribute.name === 'description'
              return (
                <div
                  key={attribute.id}
                  class={[
                    styles.attributeRow,
                    !isDescription && draggingId.value === attribute.id && styles.dragging,
                    !isDescription && dragOverId.value === attribute.id && styles.dragOver,
                  ]}
                  draggable={!isDescription}
                  onDragstart={
                    isDescription ? undefined : (e: DragEvent) => onItemDragStart(e, attribute)
                  }
                  onDragend={isDescription ? undefined : onItemDragEnd}
                  onDragover={
                    isDescription ? undefined : (e: DragEvent) => onItemDragOver(e, attribute)
                  }
                  onDragleave={isDescription ? undefined : onItemDragLeave}
                  onDrop={
                    isDescription
                      ? undefined
                      : (e: DragEvent) => {
                          const ids = onItemDrop(e, attribute, displayAttributes.value)
