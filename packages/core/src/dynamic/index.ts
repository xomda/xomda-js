...
const PRIMITIVE_BUILDERS: Record<string, () => ZodTypeAny> = {
  string: () => z.string(),
  number: () => z.number(),
  decimal: () => z.number(),
  boolean: () => z.boolean(),
...
function findEntityByName(name: string, model: Model): Entity | undefined {
  const walk = (pkgs: Package[]): Entity | undefined => {
    for (const p of pkgs) {
      const found = p.entities.find((e) => e.name === name)
      if (found) return found
      const nested = walk(p.packages)
      if (nested) return nested
    }
    return undefined
  }
  return walk(model.packages)
}

function findEnumByName(name: string, model: Model): Enum | undefined {
  const walk = (pkgs: Package[]): Enum | undefined => {
    for (const p of pkgs) {
      const found = p.enums.find((e) => e.name === name)
      if (found) return found
      const nested = walk(p.packages)
      if (nested) return nested
    }
...
function buildAttributeBase(attr: Attribute, model: Model, visiting: Set<string>): ZodTypeAny {
  ...
  const primitive = PRIMITIVE_BUILDERS[attr.type]
  if (primitive) return primitive()

  // Enum reference → z.enum(values)
  const enumDef = findEnumByName(attr.type, model)
  if (enumDef) {
  ...
  const entityDef = findEntityByName(attr.type, model)
  if (entityDef) {
