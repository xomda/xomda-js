...
function buildAttributeBase(attr: Attribute, model: Model, visiting: Set<string>): ZodTypeAny {
  // Reference (aggregation: 'none') attributes are stored as the referenced
  // element's id. Composite attributes embed the target's schema.
  if (attr.aggregation === 'none') return z.string().uuid()
