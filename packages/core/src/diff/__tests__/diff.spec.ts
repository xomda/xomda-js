...
describe('diffModels — loose extension fields', () => {
  ...
  it('treats loose objects as equal when only key insertion order differs', () => {
    // A round-trip through deserialize → serialize can reorder object keys
    // (V8 iterates own properties in insertion order; saving and reloading
    // can flip them). Equality must not see ordering — otherwise a no-op
    // save shows a spurious "diff" and triggers wasted regen / history.
    const before = base()
    const after = base()
    ;(before.packages[0].entities[0] as Record<string, unknown>).config = { x: 1, y: 2, z: 3 }
    ;(after.packages[0].entities[0] as Record<string, unknown>).config = { z: 3, y: 2, x: 1 }
    const diff = diffModels(before, after)
    expect(diff.loose.some((c) => c.kind === 'entity' && c.id === 'ent-1')).toBe(false)
  })

  it('treats `undefined`-valued loose keys as absent (no spurious diff)', () => {
    // Equivalent absence at the model level — extension key explicitly set
    // to `undefined` is structurally indistinguishable from "key missing".
    // `dequal` and the JSON-stringify ancestor agree here, but pin it.
    const before = base()
    const after = base()
    ;(after.packages[0].entities[0] as Record<string, unknown>).extra = undefined
    const diff = diffModels(before, after)
    expect(diff.loose.some((c) => c.kind === 'entity' && c.id === 'ent-1')).toBe(false)
  })

  it('still flags genuine loose changes (regression guard)', () => {
    // Sanity: the equality replacement must not flip *every* loose
    // comparison to true. Real changes still surface.
    const before = base()
    const after = base()
    ;(before.packages[0].entities[0] as Record<string, unknown>).config = { x: 1 }
    ;(after.packages[0].entities[0] as Record<string, unknown>).config = { x: 2 }
    const diff = diffModels(before, after)
    expect(diff.loose.some((c) => c.kind === 'entity' && c.id === 'ent-1')).toBe(true)
  })
})
