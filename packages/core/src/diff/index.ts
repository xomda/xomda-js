import { deepEqual } from '@xomda/util'
...
function diffFields<T extends object, K extends Extract<keyof T, string>>(
  before: T,
  after: T,
  keys: readonly K[]
): K[] {
  ...
  for (const key of keys) {
    if (!deepEqual(before[key], after[key])) changes.push(key)
  }
  return changes
}

function pickLooseKeys(obj: object, knownKeys: ReadonlySet<string>): Record<string, unknown> {
  const out: Record<string, unknown> = {}
  for (const [k, v] of Object.entries(obj)) {
    // Drop `undefined`-valued keys so they compare equal to "key absent".
    // The model serialises through JSON.stringify on disk anyway, which
    // already drops them — keep the in-memory comparison aligned so a
    // save → reload round-trip does not look like a real diff.
    if (!knownKeys.has(k) && v !== undefined) out[k] = v
  }
  return out
}
...
function recordLooseChange(
  ...
  if (!deepEqual(beforeLoose, afterLoose)) {
...
export function diffModels(before: Model, after: Model): ModelDiff {
  ...
  for (const [id, pkg] of pkgsAfter) {
    ...
    const pkgChanges = diffFields(pBefore, pkg, PACKAGE_COMPARE_KEYS)
    if (pkgChanges.length > 0) {
      diff.modified.packages.push({ id, before: pBefore, after: pkg, changes: [...pkgChanges] })
    }
    recordLooseChange(diff, 'package', id, pBefore, pkg, PACKAGE_KEYS)
  }
  ...
  for (const [id, eAfter] of entsAfter) {
    ...
    const entityChanges = diffFields(eBefore, eAfter, ENTITY_COMPARE_KEYS)
    ...
    recordLooseChange(diff, 'entity', id, eBefore, eAfter, ENTITY_KEYS)
  ...
  for (const [id, eAfter] of enumsAfter) {
    ...
    const enumChanges = diffFields(eBefore, eAfter, ENUM_COMPARE_KEYS)
    ...
    recordLooseChange(diff, 'enum', id, eBefore, eAfter, ENUM_KEYS)

    diffEnumValues(diff, eBefore, eAfter)
  }

  // ── Model-level loose fields ───────────────────────────────────────────────
  recordLooseChange(diff, 'model', before.id, before, after, MODEL_KEYS)
...
function diffAttributes(diff: ModelDiff, eBefore: Entity, eAfter: Entity): void {
  ...
  for (const [aid, aAfter] of attrsAfter) {
    ...
    const attrChanges = diffFields(aBefore, aAfter, ATTRIBUTE_COMPARE_KEYS)
    ...
    recordLooseChange(diff, 'attribute', aid, aBefore, aAfter, ATTRIBUTE_KEYS)
  }
}
...
function diffEnumValues(diff: ModelDiff, eBefore: Enum, eAfter: Enum): void {
  ...
  for (const [vid, v] of valsAfter) {
    ...
    recordLooseChange(diff, 'enumValue', vid, vBefore, v, ENUM_VALUE_KEYS)
  }
}
