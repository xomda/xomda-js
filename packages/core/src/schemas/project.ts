...
export const ProjectSettingsSchema = z
  .object({
    ...
    /**
     * When true (default), reads outside the project root are rejected —
     * specifically, the `parentProject` chain (if set) must resolve under
     * the project root. Set false explicitly when a workspace genuinely
     * needs to chain into a sibling repo. The resolver still requires the
     * target path to contain a `.xomda/` directory in either case.
     */
    restrictReadsToProjectRoot: z.boolean().default(true),
    /**
...
export const ProjectFileSchema = z
  .object({
    ...
    /**
     * Optional relative path to a parent xomda project. When set, the
     * project-graph loader walks the chain (parent → grandparent → ...)
     * and merges visible entities/enums across all projects
     * for cross-project inheritance and reference resolution. See Phase F.
     *
     * Path security: gated by `settings.restrictReadsToProjectRoot`
     * (defaults to true) — paths must resolve under the current project
     * root and the resolved directory must contain a `.xomda/` folder.
     */
