...
export const TemplateSchema = z.object({
  ...
  /**
   * UUID of the parent template this one inherits from. Mirrors the
   * `Template.extends` attribute in `.xomda/model.json`. No server-side
   * resolver yet — UI metadata only.
   */
  extends: z.string().uuid().optional(),
  /**
   * Folder path the template is stored in. Server-side persistence concern
   * (set via the move/rename routes), intentionally NOT in `.xomda/model.json`
   * because it's a filesystem-layout detail, not a domain attribute.
   */
