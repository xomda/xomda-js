...
import type { Editor, monaco as Monaco, MonacoCodeEditor, MonacoDiffEditor } from './monaco'
...
export const CodeEditor = defineComponent({
  ...
  setup(props, { emit, attrs }) {
    ...
    // monaco-editor + its 5 web workers add ~3.7 MB minified to the
    // bundle. Importing dynamically inside `onMounted` lets Rolldown
    // emit the whole module graph (monaco-editor, the workers, the
    // theme setup in `./monaco`) as a separate chunk that only loads
    // when a `CodeEditor` actually mounts. `monaco` is undefined until
    // the dynamic import resolves; every watcher that touches it also
    // checks `editor` first, which is only assigned after `monaco`
    // is loaded.
    ...
    watch(
      () => props.theme,
      () => {
        if (editor && monaco) monaco.editor.setTheme(props.theme)
      }
    )
    watch(
      () => props.readOnly,
