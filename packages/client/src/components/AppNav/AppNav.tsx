        ...
        {(() => {
          ...
          return expanded.value ? (
            ...
            <VTooltip text={themeLabel} location="right">
              {{ activator: ({ props }: { props: Record<string, unknown> }) => <button {...props} class={styles.themeToggleBtn} onClick={() => (store.darkMode = !store.darkMode)} aria-label={themeLabel}><VIcon icon={store.darkMode ? LightModeIcon : DarkModeIcon} size="18" /></button> }}
            </VTooltip>
          )
        })()}
        <button
