...
export const AppNav = defineComponent({
  ...
  setup() {
    ...
    // reload. `usePoll` auto-pauses on hidden tabs. The notification store
    // contributions are already reactive.
    const { data: loggerCount } = usePoll(() => getRecentLogs().length, 1000)
    /** True when there's anything worth surfacing in the Logs view. */
    const hasLogs = computed(
      () =>
        (loggerCount.value ?? 0) > 0 ||
