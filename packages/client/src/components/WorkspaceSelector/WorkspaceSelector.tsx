import { AddIcon, FolderXomdaIcon, ModelIcon, RefreshIcon } from '@xomda/icons'
import type { MenuItemConfig } from '@xomda/ui'
import { MenuButton, useNotificationsStore, usePrompt, Version } from '@xomda/ui'
import type { PropType } from 'vue'
import { computed, defineComponent, ref } from 'vue'
import { VBtn, VSkeletonLoader, VTooltip } from 'vuetify/components'

import type { WorkspaceProject } from '../../stores'
...
export const WorkspaceSelector = defineComponent({
  ...
  setup(props) {
    ...
    const items = computed<MenuItemConfig[]>(() => {
      ...
      // Filter by root AND name so visual duplicates (e.g. sibling checkouts
      // of the same repo discovered under a path the scan didn't exclude)
      // don't render as standalone submenus — the active project's section
      // already lists its models at the top.
      const others = store.projects.filter(
        (p) => p.root !== store.activeProjectRoot && p.name !== active?.name
      )
      if (!active) {
