...
describe('WorkspaceSelector — multi-project menu', () => {
  ...
  it('never renders a ROOT chip in the title-bar, even when the active project is isRoot', async () => {
    workspaceQueryMock.mockResolvedValueOnce(fixture())
    const w = makeWrapper()
    const store = useWorkspaceStore()
    await store.load()
  ...
  it('omits submenu entries whose name matches the active project (visual dupes)', async () => {
    // Sibling checkouts of the same repo (e.g. git worktrees outside .claude)
    // surface with the same name as the workspace root. They'd render as
    // standalone submenus that look like duplicates of the active project's
    // section at the top — filter them out so the menu doesn't repeat itself.
    workspaceQueryMock.mockResolvedValueOnce({
      workspace: project('/repo', 'root', [model('m-1', 'Main', true)], true),
      subprojects: [
        project('/elsewhere/wt-a', 'root', [model('m-a', 'A', true)], true),
        project('/elsewhere/wt-b', 'root', [model('m-b', 'B', true)], true),
        project('/repo/legit', 'legit', [model('m-l', 'L', true)]),
      ],
    })
    const w = makeWrapper()
    const store = useWorkspaceStore()
    ...
    const body = document.body.textContent ?? ''
    expect(body).toContain('Other projects')
    expect(body).toContain('legit')
    // "· ROOT" only appears on submenu entries in "Other projects"; the
    // active project's section at the top renders its name without that
    // suffix. The two same-named workspace siblings must be filtered.
