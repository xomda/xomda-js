...
import { VFadeTransition, VTextField } from 'vuetify/components'

import styles from './AppSearch.module.scss'
import type { SearchHit } from './providers'
import { createModelProvider, createTemplateProvider, createVersionProvider } from './providers'
...
export const AppSearch = defineComponent({
  ...
  setup() {
    ...
    // Deferred-blur and recompute timers: tracked so onUnmounted can clear
    // them, otherwise the callback fires against a disposed ref if the user
    // navigates away mid-arm.
    let blurDeferHandle: ReturnType<typeof setTimeout> | null = null
    let recomputeDeferHandle: ReturnType<typeof setTimeout> | null = null
    ...
    const onBlur = (e: FocusEvent): void => {
      ...
      if (blurDeferHandle !== null) clearTimeout(blurDeferHandle)
      blurDeferHandle = setTimeout(() => {
        focused.value = false
        blurDeferHandle = null
      }, 150)
    }
    ...
    onUnmounted(() => {
      ...
      if (blurDeferHandle !== null) {
        clearTimeout(blurDeferHandle)
        blurDeferHandle = null
      }
      if (recomputeDeferHandle !== null) {
        clearTimeout(recomputeDeferHandle)
        recomputeDeferHandle = null
      }
    })
    ...
    watch(focused, () => {
      ...
      if (recomputeDeferHandle !== null) clearTimeout(recomputeDeferHandle)
      recomputeDeferHandle = setTimeout(() => {
        recomputePanelPos()
        recomputeDeferHandle = null
      }, 220)
    })
      ...
      return (
        ...
        >
          <div class={styles.input}>
            ...
            >
              {{
                'append-inner': ({ isFocused }: { isFocused: Ref<boolean> }) => (
                  <VFadeTransition>
                    {isFocused.value ? undefined : <span>{`${isMac() ? '⌘' : 'Ctrl'}+K`}</span>}
                  </VFadeTransition>
                ),
              }}
            </VTextField>
