...
export const SceneMiniToolbar = defineComponent({
  ...
  props: {
    ...
    /** Recompute every package's layout — packs children deepest-first. */
    onRecalculateLayout: { type: Function as PropType<() => void>, default: undefined },
    /** Current value of the "lock pan when something is selected" preference. */
    lockPanOnSelection: { type: Boolean, default: undefined },
    /** Fired when the user toggles "lock pan when selected" on/off. */
    onLockPanOnSelectionChange: {
      type: Function as PropType<(next: boolean) => void>,
      default: undefined,
    },
      ...
      return (
        <VFadeTransition {...APPEAR_PROPS}>
          ...
          >
            ...
            {props.onLockPanOnSelectionChange && (
              <>
                <div class={styles.divider} aria-hidden="true" />
                <VTooltip
                  text={
                    props.lockPanOnSelection
                      ? 'Lock pan when something is selected — ON (click to allow panning with a selection)'
                      : 'Lock pan when something is selected — OFF (click to lock panning while selected)'
                  }
                  location="bottom"
                >
                  {{
                    activator: ({ props: tipProps }: { props: Record<string, unknown> }) => (
                      <VBtn
                        {...tipProps}
                        icon={MoveSelectionRightIcon}
                        variant="text"
                        size="small"
                        density="comfortable"
                        color={props.lockPanOnSelection ? 'primary' : undefined}
                        aria-label="Toggle lock pan on selection"
                        aria-pressed={props.lockPanOnSelection ? 'true' : 'false'}
                        onClick={() =>
                          props.onLockPanOnSelectionChange?.(!props.lockPanOnSelection)
                        }
                      />
                    ),
                  }}
            ...
            {props.onRecalculateLayout && (
              <>
                <div class={styles.divider} aria-hidden="true" />
                <VTooltip
                  text="Recalculate layout (auto-arrange every package, deepest first)"
                  location="bottom"
                >
                  {{
                    activator: ({ props: tipProps }: { props: Record<string, unknown> }) => (
                      <VBtn
                        ...
                        onClick={() => props.onRecalculateLayout?.()}
                      />
                    ),
                  }}
                </VTooltip>
