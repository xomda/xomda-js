...
describe('SceneMiniToolbar', () => {
  ...
  it('renders Recalculate layout only when onRecalculateLayout is wired and invokes it', async () => {
    const wWithout = mountToolbar({ mode: 'items' })
    expect(wWithout.find('[aria-label="Recalculate layout"]').exists()).toBe(false)
    const onRecalculateLayout = vi.fn()
    const w = mountToolbar({ mode: 'items', onRecalculateLayout })
    const btn = w.find('[aria-label="Recalculate layout"]')
    expect(btn.exists()).toBe(true)
    await btn.trigger('click')
    expect(onRecalculateLayout).toHaveBeenCalledTimes(1)
  })

  it('lock-pan toggle reflects current state and emits the inverted value on click', async () => {
    const wWithout = mountToolbar({ mode: 'items' })
    expect(wWithout.find('[aria-label="Toggle lock pan on selection"]').exists()).toBe(false)

    const onLockPanOnSelectionChange = vi.fn()
    const wOn = mountToolbar({
      mode: 'items',
      lockPanOnSelection: true,
      onLockPanOnSelectionChange,
    })
    const btn = wOn.find('[aria-label="Toggle lock pan on selection"]')
    expect(btn.exists()).toBe(true)
    expect(btn.attributes('aria-pressed')).toBe('true')
    await btn.trigger('click')
    expect(onLockPanOnSelectionChange).toHaveBeenCalledWith(false)

    const wOff = mountToolbar({
      mode: 'items',
      lockPanOnSelection: false,
      onLockPanOnSelectionChange,
    })
    const btnOff = wOff.find('[aria-label="Toggle lock pan on selection"]')
    expect(btnOff.attributes('aria-pressed')).toBe('false')
    await btnOff.trigger('click')
    expect(onLockPanOnSelectionChange).toHaveBeenLastCalledWith(true)
  })
