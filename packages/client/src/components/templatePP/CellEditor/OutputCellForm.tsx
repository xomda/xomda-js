    ...
    return () => (
      <VCardText class="d-flex flex-column ga-3 pt-4">
        <VTextField
          ...
          label="Filename"
          hint="Handlebars expression — e.g. {{pascalCase entity.name}}.ts"
          persistent-hint
          density="compact"
          variant="outlined"
        ...
        />
        <VTextField
          modelValue={props.cell.outputDirectory ?? ''}
          label="Directory"
          hint="Static path — e.g. src/entities"
          persistent-hint
          density="compact"
          variant="outlined"
          onUpdate:modelValue={(v: string) => patch({ outputDirectory: v || undefined })}
        />
        <VTextField
