...
import { useConfirm, useDelayedLoading, useModelEntity, useVersion } from '@xomda/ui'
...
export const ModelView = defineComponent({
  ...
  setup() {
    ...
    async function addAttribute() {
      ...
      try {
        ...
        model.value = await trpc.model.addAttribute.mutate({
          ...
          attribute: attribute as Parameters<typeof trpc.model.addAttribute.mutate>[0]['attribute'],
        })
    ...
    async function updateEntity() {
      ...
      if (editEntityData.value.name !== selectedEntity.value.name) {
        ...
        if (parentPkg) {
          ...
          const isDuplicate =
            parentPkg.entities.some((e) => e.name === name && e.id !== selectedEntity.value?.id) ||
    ...
    const modelVersionError = computed(() => {
      const v = (editModelData.value?.version as string | undefined) ?? ''
                    ...
                    <>
                      ...
                      <div class={styles.sidebarFooter}>
                        ...
                        <VBtn
                          variant="tonal"
                          color="primary"
                          disabled={!modelPropsDirty.value || modelVersionError.value != null}
