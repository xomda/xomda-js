...
export interface EntryDisplayProps {
  ...
  /**
   * For files only: the first plugin-declared icon (SVG path) and its
   * color, set when at least one plugin claimed this path. Replaces the
   * default file glyph so the tree node reads "Maven" or "TypeScript" at
   * a glance instead of generic-doc + decoration. Null when no plugin
   * claimed the path or when the entry is a directory.
   */
  filePrimaryIcon: string | null
  filePrimaryColor: string | null
}
...
export function getEntryDisplayProps(
  ...
  // Files inherit their primary icon from the first plugin match (priority
  // already applied server-side). Directories keep the default folder
  // glyph + project-kind overlay — replacing it would lose the folder
  // affordance.
  const filePrimary = !entry.isDirectory && kinds.length > 0 ? kinds[0] : undefined
  ...
  return {
    ...
    style: { opacity },
    projectIcons: kinds,
    filePrimaryIcon: filePrimary?.icon ?? null,
    filePrimaryColor: filePrimary?.color ?? null,
  }
}
