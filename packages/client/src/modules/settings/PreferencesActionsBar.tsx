...
import { VBtn, VSlideYReverseTransition } from 'vuetify/components'
...
export const PreferencesActionsBar = defineComponent({
  ...
  setup() {
    ...
    // Visible while there is something to save, and briefly after to let the
    // "Saved." confirmation flash run its course. Otherwise the bar slides
    // out the bottom — no actions, no chrome.
    const visible = computed(() => editor.dirty.value || justSaved.value)
      ...
      <VSlideYReverseTransition>
        {visible.value ? (
          <div class={styles.bar} role="region" aria-label="Preferences actions">
            ...
            >
              {editor.dirty.value ? 'Unsaved changes' : 'Saved.'}
            </span>
            <VBtn
              variant="text"
              onClick={onCancel}
