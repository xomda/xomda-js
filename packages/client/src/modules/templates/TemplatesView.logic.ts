...
/**
 * True when at least one open tab carries unsaved edits. Drives the
 * `beforeunload` opt-in in TemplatesView — clean state lets the browser
 * close without a prompt; any dirty buffer triggers the browser's
 * "Leave site?" dialog so the user can cancel.
 */
export function hasDirtyTabs(tabs: readonly OpenTemplateTab[]): boolean {
  return tabs.some((t) => t.buffer.dirty.value)
}
