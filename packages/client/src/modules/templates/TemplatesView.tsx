...
import type { MenuItemConfig, SortState } from '@xomda/ui'
import {
  FileEntryIcon,
  FileEntryListItem,
  MenuButton,
...
import { AppTitleBar, ViewCardHeader, WorkspaceSelector } from '../../components'
import { TemplateEditor } from '../../components/templates/TemplateEditor'
import { trpc } from '../../trpc'
import { TemplatesRoutes } from './routes'
import {
  duplicateTemplate,
  findTabsInDeletedFolder,
  hasDirtyTabs,
  newTemplate,
} from './TemplatesView.logic'
...
export const TemplatesView = defineComponent({
  ...
  setup() {
    ...
    // Browsers won't let JS swallow Cmd+W / Ctrl+W (the OS/browser sees the
    // combo first), so we can't redirect it to closeTab. The next-best thing
    // is to use `beforeunload` to give the user a chance to cancel the
    // browser-tab close when there are unsaved template edits — the same
    // safety net Google Docs and similar editors use. The browser ignores
    // any custom message and shows its own generic prompt; we just need a
    // non-empty `returnValue` to opt in.
    const onBeforeUnload = (e: BeforeUnloadEvent): void => {
      if (!hasDirtyTabs(tabs.tabs.value)) return
      e.preventDefault()
      e.returnValue = ''
    }
