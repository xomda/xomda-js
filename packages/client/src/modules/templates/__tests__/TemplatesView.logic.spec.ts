...
import { duplicateTemplate, findTabsInDeletedFolder, hasDirtyTabs } from '../TemplatesView.logic'
...
describe('hasDirtyTabs', () => {
  function dirtyTab(uuid: string): OpenTemplateTab {
    const buffer = useEditBuffer<Template>()
    buffer.set({ uuid, name: uuid, version: '1.0.0', cells: [] })
    // Mutate the draft to flip `dirty` to true.
    const draft = buffer.draft.value
    if (draft) draft.name = `${uuid}-changed`
    return { uuid, buffer: markRaw(buffer) }
  }

  it('returns false for an empty tab list', () => {
    expect(hasDirtyTabs([])).toBe(false)
  })

  it('returns false when every tab is clean', () => {
    expect(hasDirtyTabs([makeTab('a', undefined), makeTab('b', 'java')])).toBe(false)
  })

  it('returns true when any single tab is dirty', () => {
    expect(hasDirtyTabs([makeTab('a', undefined), dirtyTab('b')])).toBe(true)
  })
})
