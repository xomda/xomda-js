...
export const App = defineComponent({
  ...
  setup() {
    ...
    // Card surface knobs (Appearance preferences) are exposed as CSS
    // variables on the document root so the global `.v-card` rule in
    // packages/ui/src/styles/global.scss reads them with a fallback to
    // the legacy defaults. Writing once at startup + on every change
    // keeps everything that consumes them in sync without per-component
    // wiring.
    watch(
      () => [store.cardSurfaceAlpha, store.cardSurfaceBlur] as const,
      ([alpha, blur]) => {
        const root = document.documentElement
        root.style.setProperty('--xomda-card-bg-alpha', String(alpha))
        root.style.setProperty('--xomda-card-blur', `${blur}px`)
      },
      { immediate: true }
    )
