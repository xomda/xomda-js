...
export const STAGE_DIR = resolve(STAGE_PARENT, 'xomda-js')
 ...
 *   3. `pnpm -F xomda build` (produces dist/cli.js + assets).
...
export async function buildPublishArtifact(opts: BuildOptions = {}): Promise<BuildResult> {
  ...
  if (!opts.skipBuilds) {
    ...
    await runPnpm(['-F', 'xomda', 'build'], opts)
  } else {
  ...
  log('▶ rendering README.md / AGENTS.md / CLAUDE.md')
  await renderDocTemplates(rootPkg)
  ...
  log('▶ npm pack')
  const tarballName = `xomda-js-${version}.tgz`
...
async function stageFiles(): Promise<void> {
  // CLI bundle + library entry (dist/cli.js, dist/index.js, shared chunks)
  const xomdaDist = resolve(REPO_ROOT, 'packages', 'xomda', 'dist')
  assertExists(xomdaDist, '`pnpm -F xomda build` produced no dist/ — run a build first.')
  assertExists(
    resolve(xomdaDist, 'cli.js'),
    'dist/cli.js missing — the CLI bin entry failed to bundle.'
  )
  assertExists(
    resolve(xomdaDist, 'index.js'),
    'dist/index.js missing — the public API barrel failed to bundle.'
  )
...
async function renderDocTemplates(rootPkg: RootPackageJson): Promise<void> {
  // Top-level docs the published tarball ships. README.md is the human entry
  // point; AGENTS.md is the convention many AI agent harnesses scan for;
  // CLAUDE.md is what Claude Code reads. All three are rendered from
  // `<name>.template.md` so `{{version}}` interpolation stays consistent.
  const docs = ['README', 'AGENTS', 'CLAUDE'] as const
  for (const name of docs) {
    const tpl = await readFile(
      resolve(REPO_ROOT, 'packages', 'xomda', `${name}.template.md`),
      'utf8'
    )
    const rendered = tpl.replaceAll('{{version}}', rootPkg.version)
    await writeFile(resolve(STAGE_DIR, `${name}.md`), rendered, 'utf8')
  }
}
...
async function runCommand(cmd: string, args: string[], opts: ProcessOptions): Promise<void> {
  await new Promise<void>((resolveExit, rejectExit) => {
    // In quiet mode capture stdout to a buffer (not inherit) so it doesn't
    // pollute the test runner; on failure replay it to stderr. Without
    // this, rolldown/vite's annotated error frames (which name the
    // problematic file) get silently dropped and CI logs only show the
    // bottom of the box.
    const captureStdout = !!opts.quiet
    const child = spawn(cmd, args, {
      cwd: opts.cwd ?? REPO_ROOT,
      env: { ...process.env, ...opts.env },
      stdio: opts.quiet ? ['ignore', 'pipe', 'inherit'] : 'inherit',
      // On Windows, `pnpm`/`npm`/`npx` are `.cmd` files; spawn won't resolve
      // the extension without a shell, leading to ENOENT.
    ...
    child.on('exit', (code) => {
      if (code === 0) {
        ...
        if (captureStdout && stdoutBuf) process.stderr.write(stdoutBuf)
        rejectExit(new Error(`${cmd} ${args.join(' ')} exited with code ${code}`))
      }
    })
  })
}
