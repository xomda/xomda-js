import type { Entity, Model, ModelDiff, Template, TemplateCell } from '@xomda/core'
import { getEffectiveAttributes } from '@xomda/core'
...
function isEntityLike(value: unknown): boolean {
  return (
    typeof value === 'object' &&
    value !== null &&
    'id' in value &&
  ...
  for (const cell of cells) {
    ...
    try {
      ...
      for (const [idx, item] of items.entries()) {
        ...
        // When the iterated item is entity-shaped (own `id`/`name`/`attributes`),
        // surface its *effective* attributes alongside `attributes` (own only).
        // Templates that want interface composition + extends inheritance reach
        // for `effectiveAttributes`; existing templates that read `attributes`
        // keep their old behaviour. Applies to every loop source — entities,
        // diff-* (where items carry an `entity` field — those don't trigger
        // this, by design), and `javascript` providers that yield entities.
        if (isEntityLike(item)) {
          itemVars.effectiveAttributes = getEffectiveAttributes(
            item as Entity,
            execCtx.model as Model
          )
        }
