...
import { createTemplateFn, TEMPLATE_FN_BLOCKED_GLOBALS } from './utils'
...
const BLOCKED_GLOBALS = new Set(TEMPLATE_FN_BLOCKED_GLOBALS)
...
export const logicProcessor = defineProcessor({
  ...
  async execute(cell, ctx) {
    ...
    const fn = createTemplateFn(['scope'], body, { strict: false })
    await fn(scope)
  },
})
