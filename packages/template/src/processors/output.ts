...
export const outputProcessor = defineProcessor({
  ...
  async execute(cell, ctx) {
    const { outputType = 'file', outputFilename, outputDirectory, outputContent } = cell
    ...
    const resolvedFilename = resolveField(outputFilename, flatCtx)
    const outputPath = outputDirectory
      ? `${outputDirectory.replace(/\/+$/, '')}/${resolvedFilename}`
      : resolvedFilename
