...
import { isPrimitiveType } from '@xomda/core'
...
export function registerHelpers(hbs: typeof Handlebars = Handlebars): void {
  ...
  hbs.registerHelper(
    ...
    (attrs: Array<Attribute>, options: { data?: { root?: { name?: string } } }) => {
      ...
      const types = (attrs ?? [])
        // Reference (aggregation: 'none') fields are stored as ids (strings),
        // not embedded objects, so they don't pull in a schema/type import.
        // Anything that isn't 'none' (composite, or no aggregation set on
        // legacy primitive/enum types) goes through as embedded.
  ...
  // Reference (aggregation: 'none') self-loops don't count: stored as ids.
  hbs.registerHelper('isSelfRef', (entityName: string, attrs: Attribute[]) =>
    (attrs ?? []).some((a) => a.type === entityName && a.aggregation !== 'none')
  )
}
