...
function bootStartScript(cwd: string): Promise<{ proc: ChildProcess; stdout: string }> {
  return new Promise((resolveBoot, reject) => {
    const proc = spawn('pnpm', ['-F', '@xomda/node', 'start'], {
      ...
      // On Windows, `pnpm` is `pnpm.cmd`; spawn won't resolve the extension
      // without a shell, leading to ENOENT.
 ...
 * The script must land on the workspace root — that's where
 * `.xomda/model.json` and `.xomda/templates/` live. We get there by
 * passing `--cwd ../..` to main.ts (which calls `process.chdir` in
 * `startServer`); a shell `cd` would not be cross-platform. If a future
 * refactor drops the cwd adjustment, the homepage shows the wrong
 * project root, the templates view comes up empty, and several Cypress
 * specs fail (they look for the workspace's seeded templates). This
 * integration test catches the regression at the package-script level.
 *
 ...
 * coverage — both scripts share the `--cwd ../..` arg.
 */
describe('@xomda/node start script cwd', () => {
  ...
  it('lands on the workspace root when invoked from the worktree root', async () => {
    ...
    expect(cwdLine).not.toContain(join(WORKSPACE_ROOT, 'packages', 'node'))
  })
})
