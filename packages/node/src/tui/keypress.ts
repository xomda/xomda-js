  ...
  const commands: KeypressCommand[] = [
    {
      ...
      action: () => {
        ...
        if (url) {
          process.stdout.write(c.dim(`  opening ${url}\n`))
          openUrl(url)
        } else {
          process.stdout.write(c.yellow('  no URL available\n'))
        }
      },
    },
    {
    ...
    },
    {
      ...
      description: 'quit',
      action: () => {
        process.stdout.write(c.dim('  shutting down...\n'))
        process.exit(0)
      },
    },
    ...(options.commands ?? []),
...
function printHelp(commands: KeypressCommand[]): void {
  const lines: string[] = ['', `  ${c.bold('Shortcuts')}`]
  const keyWidth = Math.max(...commands.map((c) => c.key.length))
  for (const cmd of commands) {
    lines.push(`  ${c.green(c.bold(`press ${cmd.key.padEnd(keyWidth)}`))} ${c.dim('to')} ${cmd.description}`)
  }
  lines.push('')
