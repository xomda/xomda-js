...
export const analysisRouter = router({
  detect: publicProcedure
    ...
    .query(async ({ input }) => {
      // process.cwd() lazily — the dev/start scripts pass `--cwd ../..`
      // so startServer chdirs to the workspace root after import; a
      // module-level capture would freeze cwd at whatever it was during
      // import resolution.
