...
import { type ProjectFile, ProjectFileSchema } from '@xomda/core'
import { z } from 'zod'
...
// a value captured at module load. The node server `chdir`s into the
// repo root after imports run; if we cached process.cwd() at the top of
// this file we'd permanently see `packages/node`.
...
export const projectRouter = router({
  ...
  /**
   * Full background scan: spawns a worker thread that re-imports
   * @xomda/analysis-plugins and runs the analyzer. The response also
   * includes any nested subprojects discovered via the xomda plugin's
   * projectKind hook.
   */
  scan: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const result: AnalysisResult = await runAnalysisInWorker({
        rootPath,
        pluginModules: ['@xomda/analysis-plugins'],
      })
      const plugin = findProjectKindPlugin()
  ...
  fileTypesFor: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Baseline plugins (no `detect`, no `patterns`) are always-on
      // contributors of file-type semantics — they never appear as
      // detected features so they're never in `project.plugins`. Without
      // this bypass, enabling the user's filter would strip image/binary
      // previews and leave .png/.zip files trying to render as text.
      const baselineIds = new Set(
        getRegisteredAnalysisPlugins()
          .filter((p) => !p.detect && (!p.patterns || p.patterns.length === 0))
          .map((p) => p.id)
      )
      const allowed = new Set([...active, ...baselineIds])
  ...
  refreshPlugins: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const next: ProjectFile = ProjectFileSchema.parse(
        existing ? { ...existing, plugins: detectedIds } : { name: 'project', plugins: detectedIds }
      )
      await saveProjectMeta(next, rootPath)
      return { detectedIds, plugins: next.plugins }
    }),

  /** List every plugin registered server-side (id + display name). */
  ...
  overview: publicProcedure
    ...
    .query(async ({ input }): Promise<{ contributions: OverviewContribution[] }> => {
      ...
      // honor the allow-list (`binary`, `markdown`, …) baseline plugins
      // declare no detect/patterns and are always re-added — same shape
      // as `fileTypesFor`'s bypass at line 308.
      const baselineIds = new Set(
        getRegisteredAnalysisPlugins()
          .filter((p) => !p.detect && (!p.patterns || p.patterns.length === 0))
          .map((p) => p.id)
      ...
      const enabled =
        !active || active.length === 0 ? undefined : new Set([...active, ...baselineIds])
   ...
   * Honors the `project.plugins` allow-list (baseline plugins bypassed,
