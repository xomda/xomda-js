...
export const modelRouter = router({
  ...
  snapshot: publicProcedure
    ...
    .mutation(({ input }) => saveSnapshot(input.label)),

  listSnapshots: publicProcedure.query(() => listSnapshots()),

  diffSnapshots: publicProcedure
    .input(z.object({ before: z.string(), after: z.string() }))
    .query(async ({ input }) => {
      const [before, after] = await Promise.all([
        readSnapshot(input.before),
        readSnapshot(input.after),
      ])
      return diffModels(before, after)
    }),

  diffWithCurrent: publicProcedure
    .input(z.object({ snapshotFilename: z.string() }))
    .query(async ({ input }) => {
