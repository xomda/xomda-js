...
// ENOENT (no such file) and EISDIR (target is a directory) both surface
// as "no readable file at that path" to the caller.
...
export const fileRouter = router({
  ...
  getStats: publicProcedure.input(z.string()).query(({ input }) => {
    ...
    return {
      ...
      isXomda,
      isXomdaDir,
      // Hidden by leading-dot convention — cross-platform, matches `list`.
      isHidden: name.startsWith('.'),
      // Read-only when the owner write bit is off. Doesn't account for
      // process-effective uid/gid, ACLs, or filesystem mount flags — it's
      // a display hint, not an access-control decision.
      isReadOnly: (stats.mode & 0o200) === 0,
      // Raw POSIX permission bits (low 9). Caller formats via modeToOctal /
      // modeToSymbolic from @xomda/util.
