...
describe('projectRouter.viewsFor', () => {
  ...
  it('returns a default-wrapped view for plugins using the `preview` shorthand', async () => {
    // `binary` declares only `preview: { kind: 'image' }` on .png, so the
    // router synthesises a single default-wrapped view. (Markdown used
    // to be the example here, but it now ships explicit Preview+Source
    // views — see the next test.)
    const result = await caller.viewsFor({ path: 'logo.png', root })
    const binary = result.views.find((v) => v.pluginId === 'binary')
    expect(binary).toBeDefined()
    expect(binary?.views).toHaveLength(1)
    expect(binary?.views[0]).toMatchObject({
      id: 'default',
      label: 'Preview',
      preview: { kind: 'image' },
