...
import type { Attribute, Entity, Enum, Model, Package } from '@xomda/core'
...
const attr = (id: string, name = id, type = 'string'): Attribute => ({
  id,
  name,
  type,
  required: false,
...
const entity = (id: string, name = id, attributes: Attribute[] = []): Entity => ({
  id,
  name,
  attributes,
  kind: 'default',
...
describe('modelRouter mutations', () => {
  ...
  describe('updateEntity', () => {
    it('replaces a nested entity by id', async () => {
      ...
      const updated = await caller.updateEntity({
        entity: { id: eId, name: 'New', kind: 'default' as const, attributes: [] },
      })
    ...
    it('throws NOT_FOUND when no entity has that id', async () => {
      ...
      await expect(
        caller.updateEntity({
          entity: { id: newId(), name: 'Ghost', kind: 'default' as const, attributes: [] },
        })
  ...
  describe('reorderAttributes', () => {
    it('persists the requested attribute order to disk', async () => {
      const eId = newId()
      const aIds = [newId(), newId(), newId()]
      await seedModel(tmpDir, {
        packages: [
          pkg(newId(), {
            entities: [
              entity(eId, 'E', [attr(aIds[0], 'a'), attr(aIds[1], 'b'), attr(aIds[2], 'c')]),
            ],
          }),
        ],
      })

      const newOrder = [aIds[2], aIds[0], aIds[1]]
      const result = await caller.reorderAttributes({ entityId: eId, attributeIds: newOrder })

      expect(result.packages[0].entities[0].attributes.map((a) => a.id)).toEqual(newOrder)
      // And on disk:
      const onDisk = JSON.parse(await readFile(join(tmpDir, XOMDA_DIR, MODEL_FILE), 'utf-8'))
      expect(onDisk.packages[0].entities[0].attributes.map((a: Attribute) => a.id)).toEqual(
        newOrder
      )
    })

    it('appends attributes missing from attributeIds at the end (preserving their original order)', async () => {
      const eId = newId()
      const aIds = [newId(), newId(), newId(), newId()]
      await seedModel(tmpDir, {
        packages: [
          pkg(newId(), {
            entities: [
      ...
      // Caller only sends a partial reorder (e.g., stale client state).
      const partial = [aIds[2], aIds[0]]
      const result = await caller.reorderAttributes({ entityId: eId, attributeIds: partial })

      // c, a first (per request); b, d appended in their original relative order.
      expect(result.packages[0].entities[0].attributes.map((a) => a.id)).toEqual([
        aIds[2],
        aIds[0],
        aIds[1],
        aIds[3],
    ...
    it('throws NOT_FOUND when the entity does not exist', async () => {
      await seedModel(tmpDir, { packages: [pkg(newId())] })
      await expect(
        caller.reorderAttributes({ entityId: newId(), attributeIds: [] })
      ).rejects.toMatchObject({ code: 'NOT_FOUND' })
    })
  })
