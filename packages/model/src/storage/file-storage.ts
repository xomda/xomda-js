...
import type { Model, ModelStorage, ProjectFile, Version, VersionsIndex } from '@xomda/core'
import {
  MODEL_FILE,
  ModelSchema,
  ProjectFileSchema,
  ...
  return {
    ...
    async write(model) {
      ...
      if (resolved.exists) {
        ...
        // Normalise the incoming model through the same schema before
        // comparing. The schema's preprocessors (e.g. legacy `reference: true`
        // → `aggregation: 'none'`) only fire on parse, so without this step
        // any caller that hands us a legacy-shaped object would always look
        // different from the migrated disk-side parse, and the dirty check
        // would false-positive on every write. See
        // feedback_model_save_idempotency.md — this regression has bitten
        // us multiple times.
        const incoming = ModelSchema.parse(model)
        if (isModelUnchanged(existing, incoming)) {
          return existing
        }
      }
