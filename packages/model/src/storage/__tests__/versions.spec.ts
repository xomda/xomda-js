...
describe('versions storage', () => {
  ...
  it('rejects an upcoming version not greater than current', async () => {
    await writeModel(minimalModel())
    await expect(commitVersion({ upcomingVersion: '1.0.0' })).rejects.toThrow(/greater than 1\.0\.0/)
    await expect(commitVersion({ upcomingVersion: '0.9.9' })).rejects.toThrow(/greater than 1\.0\.0/)
  })
