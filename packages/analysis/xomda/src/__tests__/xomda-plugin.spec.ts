...
describe('xomdaPlugin.projectKind hooks', () => {
  ...
  it('listSubprojects skips .claude (worktrees and other AI-tool state)', async () => {
    // Claude Code creates git worktrees under .claude/worktrees/<name>; each
    // is a full checkout of the parent repo, so it carries the same .xomda/.
    // Without this exclude, the workspace selector lists every worktree as a
    // separate subproject sharing the parent's name — they look like dupes.
    await mkdir(join(root, XOMDA_DIR), { recursive: true })
    await mkdir(join(root, '.claude', 'worktrees', 'wt-a', XOMDA_DIR), { recursive: true })
    await mkdir(join(root, '.claude', 'worktrees', 'wt-b', XOMDA_DIR), { recursive: true })
    await mkdir(join(root, 'packages', 'real', XOMDA_DIR), { recursive: true })
