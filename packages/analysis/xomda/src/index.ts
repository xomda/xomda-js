...
import { classifyExcludes, matchesGlob, registerAnalysisPlugin } from '@xomda/analysis-core'
...
interface SubprojectFinder {
  ...
  /** Glob patterns matched against the project-relative POSIX path. */
  excludeGlobs: string[]
}

function makeFinder(excludeFromScan: readonly string[]): SubprojectFinder {
  const { basenames, paths, globs } = classifyExcludes(excludeFromScan)
  return { excludes: basenames, excludePaths: paths, excludeGlobs: globs }
}
...
function walkForSubprojects(
  ...
  for (const entry of entries) {
    ...
    if (finder.excludeGlobs.some((g) => matchesGlob(g, rel))) continue
    let isDir: boolean
    try {
...
export const xomdaPlugin: AnalysisPlugin = {
  ...
  name: 'xomda Project',
  icon: 'xomda',
  // Core: the xomda plugin owns the platform's marker (`.xomda`) and
  // contributes default file-type fallbacks (plaintext for .txt). The
  // user's project.plugins allow-list can't strip it.
  core: true,
  ...
  fileTypes: [
    ...
    // Low-priority fallback so `.txt` files preview as plaintext even
    // when no other plugin claims them. Any plugin with a higher-priority
    // match for the same path wins; keep this list narrow (no catch-all
    // glob) so we don't silently swallow file types we'd rather route
    // through a structured viewer later.
    {
      id: 'xomda-plaintext',
      label: 'Plain text',
      match: { extensions: ['.txt'] },
      preview: { kind: 'text', language: 'plaintext' },
      priority: 1,
    },
  ],
  projectKind: {
