...
describe('mavenPlugin pom.xml multi-view', () => {
  it('declares Info and Source views for pom.xml — Info first (default tab)', () => {
    const pomFileType = mavenPlugin.fileTypes?.find((f) => f.id === 'pom')
    expect(pomFileType?.views).toHaveLength(2)
    // Order matters: clients use the first view as the default tab so users
    // land on the structured dependency view, not raw XML.
    expect(pomFileType?.views?.map((v) => v.id)).toEqual(['info', 'source'])
  })
