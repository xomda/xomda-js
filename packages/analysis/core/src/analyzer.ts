...
import type { MarkerCheck, WalkOptions } from './project-walker'
import { walkForProjectKinds } from './project-walker'
import { isCorePlugin } from './registry'
import type {
  AnalysisContext,
  AnalysisPlugin,
  AnalysisResult,
...
export class ProjectAnalyzer {
  ...
  listPlugins(): Array<{ id: string; name: string; core: boolean }> {
  ...
  /**
   * Drop every cached overview + view-data entry. Use when the user
   * explicitly asks to re-scan the project (e.g. file browser toolbar
   * button) and the mtime-based invalidation alone isn't enough — for
   * example, when a plugin's logic changed without any marker file mtime
   * moving.
   */
  clearCaches(): void {
