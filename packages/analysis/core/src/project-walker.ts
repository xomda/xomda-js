...
export interface WalkOptions {
  ...
  /**
   * Glob patterns matched against the project-relative POSIX path of each
   * candidate folder (e.g. `packages/*\/dist`, `**\/tmp`).
...
export function walkForProjectKinds(
  ...
  walk(root, root, 0, maxDepth, excludes, excludePaths, excludeGlobs, checks, found)
  return found
}
...
function walk(
  ...
  for (const entry of entries) {
    ...
    if (excludeGlobs.some((g) => matchesGlob(g, rel))) continue
    let isDir: boolean
    try {
    ...
    walk(root, child, depth + 1, maxDepth, excludes, excludePaths, excludeGlobs, checks, found)
  }
}
