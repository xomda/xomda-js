...
describe('ProjectAnalyzer', () => {
  ...
  describe('listPlugins', () => {
    ...
    it('returns id, name, and core flag for each registered plugin', () => {
      const analyzer = new ProjectAnalyzer()
        // Empty patterns + no detect ⇒ core baseline (always-on contributor).
        .register({ id: 'a', name: 'Plugin A', patterns: [] })
        // Has a detect, so it's a regular detectable plugin (not core).
        .register({ id: 'b', name: 'Plugin B', detect: () => false })
        // Explicit opt-in via `core: true`.
        .register({ id: 'c', name: 'Plugin C', detect: () => false, core: true })
      expect(analyzer.listPlugins()).toEqual([
        { id: 'a', name: 'Plugin A', core: true },
        { id: 'b', name: 'Plugin B', core: false },
        { id: 'c', name: 'Plugin C', core: true },
      ])
    })
  })
