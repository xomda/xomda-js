...
describe('walkForProjectKinds', () => {
  ...
  it('honors glob patterns in excludeGlobs', () => {
    touch('packages/keep/package.json')
    touch('packages/legacy-foo/package.json')
    touch('packages/legacy-bar/package.json')
    const result = walkForProjectKinds({ rootPath: root, excludeGlobs: ['packages/legacy-*'] }, [
      { pluginId: 'node', markers: ['package.json'] },
    ])
    expect(result.map((p) => p.path)).toEqual(['packages/keep'])
  })

  it('supports `**` crossing path separators in excludeGlobs', () => {
    touch('a/b/c/dist/package.json')
    touch('a/keep/package.json')
    const result = walkForProjectKinds({ rootPath: root, excludeGlobs: ['**/dist'] }, [
      { pluginId: 'node', markers: ['package.json'] },
    ])
    expect(result.map((p) => p.path)).toEqual(['a/keep'])
  })
  ...
  it('returns the basename of the folder for both root and nested entries', () => {
    ...
    expect(byPath['.']).toBe(basename(root))
