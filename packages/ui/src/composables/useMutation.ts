...
export function useMutation<TArgs extends unknown[], TResult>(
   ...
   *
   * The `console.error` here is intentional and NOT subject to the
   * AGENTS.md ban on silent tRPC failures. By the time we reach this
   * branch the mutation itself has succeeded (or already failed and
   * notified): the user's callback is a developer-bug surface — a
   * follow-up reload that crashed, a stale invalidation, etc. The toast
   * is already up; we log here so the underlying bug isn't entirely
   * silent for the developer reading DevTools.
   */
  async function safeCallback(
    ...
    try {
      ...
      console.error(`[useMutation] ${phase} callback threw:`, callbackErr)
    }
  }
