...
export const useLocalStorageStore = defineStore('local-storage', () => {
  ...
  return {
    ...
    /**
     * Model-view option: when ON, wheel-pan is suppressed while
     * something is selected (lets the user scroll the side property
     * panel without the canvas pulling focus). Default ON — matches
     * the previous always-on behaviour; flip off to keep the canvas
     * panning even with a selection.
     */
    diagramLockPanOnSelection: computed({
      get: (): boolean =>
        store.value.diagramLockPanOnSelection === undefined
          ? true
          : !!store.value.diagramLockPanOnSelection,
      set: (value: boolean) => (store.value = { ...store.value, diagramLockPanOnSelection: value }),
    }) as Ref<boolean>,
    ...
    /**
     * Card surface opacity for the frosted-glass aesthetic, in 0..1.
     * Lower = more transparent; lets the AuroraBackground show through.
     * Default 0.78 matches the legacy hard-coded value in global.scss.
     * Local-only so each user can dial the chrome to taste without
     * shifting it for collaborators on the same project.
     */
    cardSurfaceAlpha: computed({
      get: (): number => {
        const raw = store.value.cardSurfaceAlpha
        const n = typeof raw === 'number' ? raw : Number(raw)
      ...
      set: (value: number) =>
        (store.value = {
          ...store.value,
          cardSurfaceAlpha: Math.min(1, Math.max(0, value)),
        }),
    }) as Ref<number>,

    /**
     * Backdrop blur radius applied to card surfaces, in CSS pixels.
     * 0 = no blur (cheaper, sharper); ~8 = legacy default; higher
     * values smear the background further. Clamped to 0..40 to keep
     * the renderer from grinding to a halt on weak GPUs.
     */
    cardSurfaceBlur: computed({
      get: (): number => {
        const raw = store.value.cardSurfaceBlur
        const n = typeof raw === 'number' ? raw : Number(raw)
      ...
      set: (value: number) =>
        (store.value = {
          ...store.value,
          cardSurfaceBlur: Math.min(40, Math.max(0, value)),
        }),
    }) as Ref<number>,
