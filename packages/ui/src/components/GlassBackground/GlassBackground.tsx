...
const LIGHT_TARGET = { lightL: 99, midL: 92, darkL: 82, ambient: 0.55, vignetteAlpha: 0.05 }
...
export const GlassBackground = defineComponent({
  ...
  setup(props) {
    ...
    try {
      ...
      // No Vuetify in tree (e.g. isolated unit test) — stays light unless `mode` overrides.
    }
    ...
    function render(t: number) {
      ...
      const vig = ctx.createRadialGradient(W / 2, H / 2, Math.min(W, H) * 0.3, W / 2, H / 2, Math.max(W, H) * 0.78)
    ...
    function loop(now: number) {
      ...
      lastFrame = now
      phaseT += dt * props.animationSpeed
      render(phaseT)
    ...
    watch(
      ...
      () => {
        if (!ctx) return
        rebuildMesh()
        if (raf == null) render(phaseT)
      }
    )

    watch([isDark, () => props.intensity], () => {
      if (ctx && raf == null) render(phaseT)
    })
