...
export const MenuButton = defineComponent({
  ...
  setup(props, { emit, attrs, slots }) {
    ...
    return () => {
      ...
      // Only wrap the activator in a <span> when we need the `labeledHover`
      // style hook. An unconditional wrapper breaks `VBtnGroup`'s
      // `> .v-btn` selectors — the chevron button loses its sibling sizing
      // and shrinks below the labelled button next to it.
      const wrap = (node: JSX.Element) =>
        labelled ? <span class={styles.labeledHover}>{node}</span> : node
      return (
        <VMenu
          modelValue={open()}
                ...
                <VTooltip text={props.tooltip} location="bottom" disabled={open()}>
                  {{
                    activator: ({ props: tipProps }: { props: Record<string, unknown> }) =>
                      wrap(renderActivator(menuProps, tipProps)),
                  }}
                </VTooltip>
              ) : (
                wrap(renderActivator(menuProps))
              ),
            default: () => (
