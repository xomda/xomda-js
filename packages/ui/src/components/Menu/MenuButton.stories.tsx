...
//
// The story declares an ad-hoc `onModelChange` arg (not a MenuButton
// prop) so Storybook's Actions panel logs which model the user picks.
// We carry that through with a story-local args type rather than
// `as unknown as`.
type LabeledArgs = { onModelChange?: (name: string) => void }

export const Labeled: StoryObj<LabeledArgs> = {
  args: { onModelChange: fn() },
  render: (args) => ({
    setup() {
      const onClick = (name: string) => () => args.onModelChange?.(name)
      return () => (
        <MenuButton
