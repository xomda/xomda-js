import type { Attribute, Entity, Model, Package } from '@xomda/model'
...
export interface FieldRenderContext {
  attribute: Attribute
  value: unknown
  /** Update a single field's value (does NOT trigger commit). */
  onUpdate: (next: unknown) => void
  /** Signal that this field should be persisted. Typically called on blur or change. */
  onCommit: () => void
}
...
export const DynamicForm = defineComponent({
  ...
  setup(props, { emit }) {
    const renderField = (attr: Attribute): VNode | null => {
      ...
      const commit = () => emit('commit', attr.name)

      const override = props.fieldOverrides[attr.name]
      if (override) {
        return override({ attribute: attr, value, onUpdate: update, onCommit: commit })
      }
