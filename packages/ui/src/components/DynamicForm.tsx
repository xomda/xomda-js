import type { Attribute, Entity, EntityKind, Model, Package } from '@xomda/core'
import { getEntityKind, sortAttributesForDisplay } from '@xomda/core'
...
/** Allowed parent kind for `extends` based on the child entity's kind. */
 ...
 *
 * When `entityKindFilter` is supplied and the type is 'Entity', only entities
 * whose effective kind matches the filter are returned — used to gate
 * `extends` (must target abstract / interface) and `implements` (must target
 * interface) pickers.
 */
function collectInstancesByType(
  typeName: string,
  model: Model,
  options: { entityKindFilter?: EntityKind } = {}
  ...
  const walk = (pkg: Package): void => {
    if (typeName === 'Entity') {
      for (const e of pkg.entities) {
        if (
          options.entityKindFilter !== undefined &&
          getEntityKind(e) !== options.entityKindFilter
        ) {
          continue
        }
...
/**
 * Heuristic kind-filter for known Entity meta-attributes:
 *   - `implements`: target must be `interface`.
 *   - `extends`:    target must match the child's allowed parent kind.
 *
 * Returns `undefined` (no filter) for any other Entity-typed attribute.
 */
function inferEntityKindFilter(attr: Attribute, modelValue: JsonObject): EntityKind | undefined {
  if (attr.type !== 'Entity') return undefined
  if (attr.name === 'implements') return 'interface'
  if (attr.name === 'extends') {
    const childKind = (modelValue.kind as EntityKind | undefined) ?? 'default'
    return ALLOWED_PARENT_KIND[childKind]
  }
  return undefined
}
...
export const DynamicForm = defineComponent({
  ...
  setup(props, { emit }) {
    const renderField = (attr: Attribute): VNode | null => {
      ...
      // For Entity-typed `extends` and `implements`, items are kind-filtered so
      // the picker matches the schema's superRefine rules.
      if (attr.aggregation === 'none' && !PRIMITIVE_TYPES.has(attr.type) && props.model) {
        const entityKindFilter = inferEntityKindFilter(attr, props.modelValue)
        const items = collectInstancesByType(attr.type, props.model, { entityKindFilter })
          .filter((it) => {
        ...
        // Multi-valued reference → multi-select with chips (e.g. Entity.implements).
        if (attr.multiValue) {
    ...
    return () => (
      <div>
        {sortAttributesForDisplay(props.entity.attributes).map((attr) => {
