...
describe('Home – dashboard', () => {
  ...
  it('opens the project-properties side panel when the hero title is clicked', () => {
    cy.get('[role="dialog"][aria-label="Edit project"]').should('not.exist')

    cy.get('h1 button[aria-label="Edit project name and description"]')
      .should('have.attr', 'aria-expanded', 'false')
      .click()

    cy.get('[role="dialog"][aria-label="Edit project"]').should('be.visible')
    cy.get('h1 button[aria-label="Edit project name and description"]').should(
      'have.attr',
      'aria-expanded',
      'true'
    )

    // Clicking the title a second time closes the panel.
    cy.get('h1 button[aria-label="Edit project name and description"]').click()
    cy.get('[role="dialog"][aria-label="Edit project"]').should('not.exist')
  })
