...
export async function addMavenProject(
  ...
  const generated = resolve(source, 'src', 'main', 'generated')
  const isUnder = (path: string, dir: string) => path === dir || path.startsWith(`${dir}${sep}`)
  await cp(source, dest, {
    recursive: true,
    force: true,
    filter: (src) => !isUnder(src, nodeModules) && !isUnder(src, generated),
  })
}
