...
describe('vitest.config.ts root projects list', () => {
  ...
  // The declared list uses POSIX separators (the repo's vitest.config.ts is
  // committed with `/`). `path.relative` returns OS-native paths — on Windows
  // that's `\`, which never matches the declared `/` paths. Normalise so the
  // spec passes on the Windows CI runner.
  const discovered = findConfigsWithTestBlock(REPO_ROOT).map((p) =>
    relative(REPO_ROOT, p).split(sep).join('/')
  )
