...
export function resolveWorkspaceRelative(
  ...
  // aren't part of the on-disk path. Plain index/slice instead of a
  // regex so the work is unambiguously linear in input length.
  ...
  const noQuery = cut === normalised.length ? normalised : normalised.slice(0, cut)
