...
public class LogicProcessor implements CellProcessor {
    ...
    public void execute(TemplateCell cell, CellContext ctx) {
        ...
        Set<String> beforeKeys = new HashSet<>(bindings.getMemberKeys());

        try {
            Value result = graalCtx.eval(Source.newBuilder("js", iife, "<logic>").build());

            // Bare assignments inside the IIFE (e.g. `name = 'Entity'`) become globals
            // in sloppy mode. Pull any newly-introduced globals back into the cell context
            // so subsequent cells (e.g. output cells) can reference them by name.
            for (String key : bindings.getMemberKeys()) {
                if (beforeKeys.contains(key))
                    continue;
                Object javaVal = toJavaValue(bindings.getMember(key));
                ctx.getVariables().put(key, javaVal);
                ctx.getState().getContextDiff().put(key, javaVal);
            }
