...
/**
 * Loop cells are handled by TemplateEngine (it drives recursive iteration
 * over the cell's children once per yielded item). The processor itself is
 * a no-op marker so PROCESSORS.get(LOOP) still resolves.
 */
