...
import org.xomda.generator.template.OutputType;
...
public class OutputProcessor implements CellProcessor {
    ...
    public void execute(TemplateCell cell, CellContext ctx) throws IOException {
        OutputType outputType = cell.getOutputType() != null ? cell.getOutputType() : OutputType.FILE;
        ...
        if (outputType == OutputType.CONTEXT) {
            ...
            String varName = cell.getOutputContent();
            if (varName != null && !varName.isEmpty()) {
                ctx.getVariables().put(varName, aggregated);
            }
        } else {
            ...
            emitFile(cell, ctx, aggregated);
        }
    }

    private void emitFile(TemplateCell cell, CellContext ctx, String aggregated) throws IOException {
