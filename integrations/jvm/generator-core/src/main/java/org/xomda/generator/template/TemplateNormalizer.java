...
/**
 * Migrates legacy flat-shape templates into the hierarchical loop+children shape used by the current engine. Mirrors
 * the TypeScript {@code normalizeTemplate} so JVM consumers can load old templates too.
 *
 * Cell-type tolerance ("provider" → LOOP, "provider-logic" → LOOP_LOGIC) is already handled by
 * {@link CellType#fromValue(String)}; this class handles the structural migration (siblings after a loop become
 * children).
 */
public final class TemplateNormalizer {
    ...
    public static void normalize(Template template) {
        if (template == null || template.getCells() == null)
            return;
