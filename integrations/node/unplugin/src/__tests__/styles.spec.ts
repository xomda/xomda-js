...
describe('xomdaStylesPlugin', () => {
  ...
  it('redirects the post-alias path even when it arrives with Windows backslashes', () => {
    // Regression: vite's aliaser hands plugins the post-alias path in
    // native-separator form, so on Windows we see backslashes. The id
    // set is posix-normalized — without normalizing the incoming source
    // too, the plugin failed to intercept and rolldown tried to load a
    // file that doesn't exist ("system cannot find the file specified").
    const plugin = xomdaStylesPlugin({
      packages: { '@xomda/ui': 'D:\\abs\\path\\to\\packages\\ui\\src' },
    })
    const resolveId = plugin.resolveId as unknown as ResolveId
    expect(resolveId('D:\\abs\\path\\to\\packages\\ui\\src\\style.css')).toBe(
      '\0xomda:empty-styles.css'
    )
    // And the bare specifier still works under a Windows srcDir.
    expect(resolveId('@xomda/ui/style.css')).toBe('\0xomda:empty-styles.css')
  })
