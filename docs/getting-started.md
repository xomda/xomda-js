...
By default xomda uses the `.xomda/` folder in the current working directory. Override with the `XOMDA_DIR` environment
variable.
...
1. A **handlebars** child cell:
   ...
   export interface {{pascalCase name}} {
     {{#each attributes}}{{camelCase name}}: {{type}}
     {{/each}}
   }
   ```
...
entity. (Drop the output cell *after* the loop instead if you want a single bundled file containing every entity.)

In the **Generate** view, click *Generate*. xomda runs the template once per entity in the model and writes the
