...
The wrapper is purely additive. xomda continues to work without it: `npx xomda-js generate`, a global install of
`xomda-js`, or a project devDependency on `xomda-js` all keep working. (The npm package is `xomda-js`; the installed
CLI binary is `xomda`.)
...
└── .xomda/
    ...
    └── wrapper/
        ...
        └── node_modules/xomda-js/… # bootstrapped install      ← gitignored
```
...
```bash
npx xomda-js wrapper                 # pins the version of xomda you just ran
npx xomda-js wrapper --pin 1.2.3     # or pin an explicit version
npx xomda-js wrapper --force         # overwrite existing xomdaw / xomdaw.cmd
```
...
2. If `.xomda/wrapper/node_modules/xomda-js/` is missing or its version doesn't match the pin, it writes a minimal
   `package.json` next to it and runs `npm install` to fetch the pinned `xomda-js` from the npm registry.
3. It resolves the `bin.xomda` entry from the installed package and execs `node .../xomda-js/<bin>` with your arguments.
...
## Why not just `npx xomda-js@<version>`?
...
If those aren't constraints you have, skip the wrapper — `npx xomda-js@<version>` is fine.
