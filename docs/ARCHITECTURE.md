...
**Purpose**: Notebook-style template engine. A template is a sequence of *cells*, each cell processed by a dedicated
processor. Cells share a context: logic cells set variables, handlebars cells consume them and emit text, output cells
declare files. Provider cells turn one template into per-entity / per-enum / per-package / per-arbitrary-item runs.

**Template shape** (`@xomda/core` schema):

```typescript
type CellType = 'logic' | 'markdown' | 'handlebars' | 'buffer' | 'output' | 'provider' | 'provider-logic'

interface TemplateCell {
  uuid: string
  type: CellType
  content: string
  variableName?: string       // logic / handlebars / provider: name to expose result under
  ...
  providerSource?: 'entities' | 'enums' | 'packages' | 'javascript'
}
...
interface Template {
  ...
  description?: string
  version: string
  scope?: 'Entity' | 'Enum' | 'Package'  // legacy fallback when no provider cell present
  folder?: string
  cells: TemplateCell[]
}
...
- `logic` — JavaScript executed against the current context (GraalVM in the Java runtime; sandboxed `Function` in Node). Assigns variables (`fields = ...`) that subsequent cells can read.
- `handlebars` — Handlebars template string rendered against the current context; rendered output appended to the cell's `$out` buffer and (if `variableName` set) exposed as a variable.
- `output` — Declares an emitted file. `outputFilename` is itself a Handlebars expression (e.g. `{{pascalCase name}}.java`); content is either the value of the variable named by `outputContent`, or the concatenation of all preceding cell `$out` buffers.
- `provider` — Iterates a model collection (`entities`, `enums`, or `packages`, walking nested packages). Causes the rest of the cells to run once per item, with `variableName` (default `item`) and the item's fields spread into context.
- `provider-logic` — Like `provider` but the items come from arbitrary JavaScript that returns a list.
...
1. If a `provider` (or `provider-logic`) cell is present → execute the template once per item it yields.
2. Else if `template.scope` is `Entity` / `Enum` / `Package` → execute once per item from the model's flattened collection (legacy path; provider cell is preferred for new templates).
...
These are *generic*. Target-language type maps (Java types, Zod types, etc.) live in `logic` cells inside individual templates, not in helpers.
...
- Each cell processor receives a `CellContext` with `model`, `scopeContext`, mutable `variables`, the array of `cellBuffers`, and a fresh `$out` buffer.
