...
export const AttributeSchema = z
  .object({
    ...
    aggregation: z.enum(['composite', 'none']).optional(), // UML aggregation kind; entity-typed attrs only
    description: z.string().optional(),
...
**Rendering** (`renderer.ts → renderTemplate`): executes the template tree once with model context. Iteration happens
inside the engine via `loop` / `loop-logic` cells.
