...
Shipped:
- **G1 [`demo/maven-plain`](../demo/maven-plain/)** — pure Java records, Maven, JUnit 5.
- **G3 [`demo/gradle-plain`](../demo/gradle-plain/)** — same model + template as G1,
  Gradle + JUnit 5; build.gradle adds src/main/generated as a Java source root.
- **G2 [`demo/springboot-blog`](../demo/springboot-blog/)** — Spring Boot 4, JPA, Flyway,
  `@SpringBootTest` against H2.
- **G4 [`demo/quarkus-todos`](../demo/quarkus-todos/)** — Quarkus 3 + Panache + JAX-RS,
  `@QuarkusTest` + REST-assured against H2.
- **[`demo/vue-blog`](../demo/vue-blog/)** — Vue 3 + Vite SPA consuming generated
  Zod schemas; Vitest node + happy-dom split.

Open:
- **G5 `demo/spring-elastic`** — persistence-layer swap + the first generated
  *test* code (entity smoke tests authored as templates).

Open questions before G5: whether generated tests live in `src/test/generated/`
(gitignored) or are commit-then-edit; CI gating for Testcontainers
Postgres/Elastic when those land. (Per-demo model already decided —
each demo owns its `.xomda/model.json`.)

See [`docs/.backlog/demo-projects.md`](./.backlog/demo-projects.md) for the full matrix.
...
- Pick-meta-type UX in _Add_ dialogs: when multiple concrete entity types exist, show a picker for concrete types
  only. Entities of kind `abstract` or `interface` excluded from instantiation; single concrete type → direct
  instantiation. (Entity kinds shipped in C; this UX is the consumer of that schema.)
- `DynamicForm` rendering of effective attributes: inherited attributes shown with visual distinction; child entities
  may override. The template engine surfaces `effectiveAttributes` (extends + blueprint composition) — the UI form
  needs to read it and badge the source per row.
- `BlueprintEditor` UI: a dedicated tree node and editor for Blueprints (today they only edit through the generic
  DynamicForm via the Entity-side `includes` picker). Sibling of EntityEditor / EnumEditor.

### Attribute sub-entities (the richer version of `config`)

Per-type extension fields shipped in Phase E for the common cases:
`targetEntity`, `targetEnum`, `length`, `precision`, `scale`. The richer
vision below addresses the long-tail config space that does not fit into
those typed fields:

- `Attribute.config: Record<string, unknown>` stays as the open container
  for tier-2 custom keys.
- A dedicated **attribute package** holding reusable sub-entity
  definitions that attributes reference by id, e.g. `ReferenceConfig`
  (onDelete, fkColumn), `ValidationRule` (minLength, pattern). Blocked on:
...
- Migration impact analysis between model versions.

Shipped in the brainstorm-reconciliation cycle:
- ~~Circular dependency detection~~ — `ModelSchema.superRefine` rejects
  cycles across the combined `extends` + `implements` graph (C4); the
  project-graph layer (F) extends cycle detection to cross-model and
  cross-project edges.
- ~~Type consistency validation~~ — `AttributeSchema.superRefine` enforces
  per-type extension-field rules (E4); `ModelSchema.superRefine` enforces
  the child-kind → parent-kind table for `extends` (C4).
- ~~Orphaned reference detection (cross-model)~~ — `validateModelAgainstGraph`
  emits `extends-target-not-found`, `implements-target-not-found`,
  `includes-target-not-found` for unresolved UUIDs (F).
- ~~Cross-reference validation for `reference: true` attributes~~ —
  Attributes with `type: 'entity'` / `type: 'enum'` reach the
  project-graph resolver via `targetEntity`/`targetEnum` UUIDs; visibility
  is enforced (F).
