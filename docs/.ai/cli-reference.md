...
```bash
npx xomda-js [subcommand] [options]   # no install required
./xomdaw [subcommand] [options]       # if the user has run `xomda wrapper`, pinned version
```

The bundled `xomda` is **a single npm package** named **`xomda-js`** — the
CLI binary it installs is `xomda` (so `npx xomda-js` and `xomda <subcommand>`
both work after `npm install xomda-js`). Internal `@xomda/*` packages are not
