...
```ts
{
  ...
  description?: string,
  // `kind`, `implements`, and `visibility` exist on the runtime schema —
  // see the inheritance + visibility sections of docs/data-model.md.
}
```
...
```ts
{
  ...
  aggregation?: 'composite' | 'none', // UML aggregation kind; entity-typed attrs only
  targetEntity?: UUID,      // only when type === 'entity' — points at the referenced entity
  targetEnum?: UUID,        // only when type === 'enum' — points at the referenced enum
  length?: number,          // codegen hint for string columns (e.g. VARCHAR length)
  precision?: number,       // codegen hint for decimal columns (total digits)
  scale?: number,           // codegen hint for decimal columns (digits after the point)
...
### Aggregation × multiValue → cardinality

`aggregation` is a UML 2 modelling primitive, only meaningful when `type`
resolves to another Entity. Forbidden on primitive and `'enum'` types
(parse error). Combined with `multiValue`, it captures all four MDA
cardinalities:

| `multiValue` | `aggregation` | Relationship |
| --- | --- | --- |
| `true`  | `'composite'` | one-to-many (composition) — parent owns the collection |
| `true`  | `'none'`      | many-to-many (association) — shared references |
| `false` | `'composite'` | one-to-one (composition) — parent owns the single child |
| `false` | `'none'`      | many-to-one (association) — pointer to a separately-owned entity |

When omitted on an entity-typed attribute, the runtime treats it as
composite (embedded). The meta-model declares `defaultValue: 'none'` so
the meta-driven form seeds new attributes with the idiomatic MDA
association.

The legacy `reference: boolean` field is migrated automatically on
read: `reference: true` → `aggregation: 'none'`, `reference: false` on
an entity type → `aggregation: 'composite'`. Writes emit the new field
only.
      ...
      "entities": [
        {
          ...
          "attributes": [
            ...
            { "id": "44444444-4444-4444-8444-444444444444", "name": "author", "type": "Author",  "aggregation": "none", "required": true }
          ]
        },
        {
