...
| Type         | Purpose                                                                                                                                                                                                                                                         |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `loop`       | Declares the iteration unit. Picks a model collection (`entities`, `enums`, `packages`, `blueprints`, `models`, `projects`) or runs JavaScript that returns an item list. The cell's `children` execute once per yielded item, with the iteration variable bound. Loops may be nested inside loops. |
...
A template without any loop cell runs once over the whole model. Use loop cells to iterate.
...
| `loopSource`     | Loop cell only: `entities`, `enums`, `packages`, `blueprints`, `models`, `projects`, `javascript`, or one of the `diff-*` sources.                          |
...
## Loop sources and scope variables

A loop cell binds its iteration item to `variableName` (default `item`) AND
spreads the item's own properties into the surrounding scope. For an
entity-shaped item, that means subsequent cells can read `name`,
`attributes`, `kind`, `implements`, `includes`, `visibility`, etc. directly.

The engine also surfaces these synthetic scope variables when the iterated
item looks like an entity:

| Scope variable        | What it carries                                                                                                                                       |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `attributes`          | The entity's own attribute list (unchanged).                                                                                                          |
| `effectiveAttributes` | Own attributes plus attributes inherited via `extends` plus attributes folded in from every Blueprint listed in `includes`. Resolves conflicts by name (own > later blueprint > earlier blueprint > closer ancestor > more distant ancestor). Use this when emitting generated types — it is the shape the entity actually has at runtime. |

`effectiveAttributes` is set on every entity-shaped item, regardless of
which loop source produced it (the built-in `entities` source, a custom
`javascript` provider that yields entities, or a `diff-*` payload that
carries entity objects). Existing templates that read `attributes` keep
their old behaviour.

For Blueprints themselves, the `blueprints` source iterates every Blueprint
declared on the model — top-level (`model.blueprints`) and per-package
(`package.blueprints`) — in declaration order.
