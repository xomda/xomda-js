...
To generate one file containing every entity's schema, drop the output cell *after* the loop (as a sibling, not a
child):
...
1. **Author a template** in the *Templates* view (or by adding a `*.template.json` file under `.xomda/templates/`).
2. **Generate** in the *Generate* view. xomda runs every template against the current model and writes the output
   files to disk.
