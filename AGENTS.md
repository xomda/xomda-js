...
37. **Keep [`CHANGELOG.md`](./CHANGELOG.md) current — concise, not storytelling.** Every user-facing change adds **one
    line** under `## [Unreleased]`, same commit. Tag it `[feature]` / `[fix]` / `[change]` / `[deprecate]` / `[remove]` /
    `[security]`; prefix breaking changes with `*breaking:*`. No prose, no examples, no per-group headers. On release,
    rename `[Unreleased]` to `version - date`, add a fresh empty `[Unreleased]`, bump compare links. SemVer. Skip only
    purely-internal changes (CI, behaviour-neutral refactors); note "no changelog: internal" in the commit when you skip.
